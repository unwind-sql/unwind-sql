...
👉 Voir [models/macros/apply_fee.sql](models/macros/apply_fee.sql)

## 3. Le Pipeline (Idéal pour l'optimisation)
...
👉 Voir [models/int_order_base.sql](models/int_order_base.sql)

### Étape 2 (démo `@disabled`) : Déduplication court-circuitée

**Table : `int_orders_dedup`** — annotée `-- @disabled: true`, ce no-op est
laissé volontairement dans le DAG pour montrer le mute Blender-style : Unwind
saute le corps SQL et alias la vue sur son premier parent (`int_order_base`).
Désactivez la directive pour réactiver le `DISTINCT *`.

👉 Voir [models/int_orders_dedup.sql](models/int_orders_dedup.sql)

### Étape 3 : Calcul des Coûts de Transport de Base (Candidat Fusion)

**Table : `int_transport_costs`**
On calcule le coût de base, puis on utilise la macro pour la surcharge carburant.

👉 Voir [models/int_transport_costs.sql](models/int_transport_costs.sql)

### Étape 4 : Calcul des Taxes Locales (Candidat Fusion)

**Table : `int_tax_costs`**
On ajoute la couche fiscale. On lit depuis la table précédente, mais on reste sur le même grain (order_id).

👉 Voir [models/int_tax_costs.sql](models/int_tax_costs.sql)

### Étape 5 : Marge Nette par Commande (Jointure Tardive)

**Table : `int_net_margin_per_order`**
On intègre les remboursements éventuels et on fait l'équation finale.

👉 Voir [models/int_net_margin_per_order.sql](models/int_net_margin_per_order.sql)

### Étape 6 : Agrégation (Résultat Final)

**Table : `fct_warehouse_profitability`**

👉 Voir [models/fct_warehouse_profitability.sql](models/fct_warehouse_profitability.sql)
